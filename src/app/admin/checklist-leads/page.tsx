...
export default function ChecklistLeadsAdminPage() {
    ...
    async function handleCopyEmail(lead: ChecklistLead) {
        try {
    ...
    async function handleToggleEmailSent(lead: ChecklistLead) {
        ...
        try {
            ...
            const data = (await response.json()) as { lead?: ChecklistLead; error?: string };
            if (!response.ok) {
            ...
            if (data.lead) {
                ...
                setLeads((current) =>
                    current.map((item) =>
                        item.id === lead.id
                            ? { ...item, emailSentAt: nextSent ? new Date().toISOString() : null }
                            : item
                    )
                );
            }
    ...
    async function handleDelete(lead: ChecklistLead) {
        const confirmed = window.confirm(
            `Delete lead for ${getLeadName(lead)} (${lead.email})?`
        );
        if (!confirmed) return;
        ...
        <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "2rem 1.25rem 3rem" }}>
            ...
            >
                ...
                <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
                    ...
                    <Link href="/admin/hall-of-fame" style={navButtonStyle}>
                        Hall of Fame
                    </Link>
                    <button type="button" onClick={handleLogout} style={navButtonAsButtonStyle}>
                        Log out
                    </button>
                </div>
            </div>
                ...
                {leads.map((lead) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            >
                                <div>
                                    <strong>Full name:</strong> {getLeadName(lead)}
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    {lead.industryTitle || lead.industrySlug}
                                </div>
                                {sent && lead.emailSentAt ? (
                                    <div>
                                        <strong>Sent on:</strong> {formatDate(lead.emailSentAt)}
                                    </div>
                                ) : null}
                            </div>
                            ...
                            >
                                ...
                                <button
                                    type="button"
                                    onClick={() => handleToggleEmailSent(lead)}
                                    disabled={updatingId === lead.id}
                                    style={sent ? undoSentButtonStyle : markSentButtonStyle}
                                >
                                    {updatingId === lead.id
                                        ? "Updating..."
                                        : sent
...
const navButtonAsButtonStyle: CSSProperties = {
    ...navButtonStyle,
    cursor: "pointer",
};
...
const markSentButtonStyle: CSSProperties = {
    padding: "0.55rem 1rem",
    borderRadius: "8px",
...
const undoSentButtonStyle: CSSProperties = {
    padding: "0.55rem 1rem",
    borderRadius: "8px",
